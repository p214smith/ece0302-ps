...
TEST_CASE("Simple contains test", "[frontier_queue]")
{
    ...
    REQUIRE(fq.contains(3));
    REQUIRE(fq.contains(4));
    REQUIRE(fq.contains(5));
    REQUIRE_FALSE(fq.contains(6));
}
...
TEST_CASE("Simple pop test", "[frontier_queue]")
{
    ...
    REQUIRE(state.getPathCost() == 2);
    REQUIRE(state.getFCost() == 3);
    state = fq.pop();
    REQUIRE(state.getValue() == 3);
    REQUIRE(state.getPathCost() == 3);
    REQUIRE(state.getFCost() == 4);
    state = fq.pop();
    REQUIRE(state.getValue() == 4);
    REQUIRE(state.getPathCost() == 4);
    REQUIRE(state.getFCost() == 5);
    REQUIRE(fq.empty());
}
